...
class DryScaffoldGenerator < DryGenerator
  ...
  def index_path
    return "collection_path" if options[:resourceful]
    "#{collection_name}_path"
  end
  
  def new_path
    return "new_resource_path" if options[:resourceful]
    "new_#{singular_name}_path"
  end
  
  def show_path(object_name = resource_instance)
    return "resource_path(#{object_name})" if options[:resourceful]
    "#{singular_name}_path(#{object_name})"
  end
  
  def edit_path(object_name = resource_instance)
    return "edit_resource_path(#{object_name})" if options[:resourceful]
    "edit_#{show_path(object_name)}"
  end
  
  def destroy_path(object_name = resource_instance)
    return "resource" if options[:resourceful]
    "#{object_name}"
  end
  
  def index_url
    return "collection_url" if options[:resourceful]
    "#{collection_name}_url"
  end
  
  def new_url
    return "new_resource_url" if options[:resourceful]
    "new_#{singular_name}_url"
  end
  
  def show_url(object_name = resource_instance)
    return "resource_url(#{object_name})" if options[:resourceful]
    "#{singular_name}_url(#{object_name})"
  end
  
  def edit_url(object_name = resource_instance)
    return "edit_resource_url(#{object_name})" if options[:resourceful]
